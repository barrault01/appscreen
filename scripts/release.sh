...
BUNDLE_ID="com.barrault.appstorescreen"
...
TAG="v$VERSION"
ZIP_NAME="AppStoreScreens-$VERSION-aarch64.zip"
...
# Check for asc (optional)
if command -v asc &>/dev/null; then
  echo "  ✓ asc ($(command -v asc))"
  if asc auth check &>/dev/null; then
    echo "  ✓ asc authenticated"
  else
    echo "  ⚠ asc not authenticated — run: asc auth setup"
  fi
else
  echo "  ⚠ asc not found — notarization will be skipped"
fi
...
PATH="$HOME/.cargo/bin:$PATH" npx @tauri-apps/cli build 2>&1 | grep -E "Compiling appstore|Finished|Bundling.*\.app|error\[" || true
...
if [ -n "$SIGNING_IDENTITY" ]; then
  ...
  if command -v asc &>/dev/null && asc auth check &>/dev/null 2>&1; then
    ...
    if echo "$NOTARIZE_OUTPUT" | grep -qi "accepted\|success"; then
      ...
      echo "  Continuing with unsigned zip..."
    fi
  else
...
if gh release view "$TAG" &>/dev/null; then
  ...
  gh release upload "$TAG" "$ZIP_PATH#$APP_NAME (macOS Apple Silicon)" "$BUNDLE_DIR/latest.json"
  echo "  ✓ Updated"
else
